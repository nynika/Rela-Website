...
function Registration() {
  ...
  const [binarydatafs, setBinarydatafs] = useState("");
  ...
  const handleFileSelect = (event) => {
    ...
    frontSideReader.readAsArrayBuffer(frontSide);
    frontSideReader.onload = (event) => {
      const arrayBuffer = event.target.result;
      const base64String = arrayBufferToBase64(arrayBuffer);
      setBinarydatafs(base64String);
    };
    frontSideReader.onerror = (error) => {
  ...
  const arrayBufferToBase64 = (arrayBuffer) => {
    //   console.log("base64 string : ",setBinarydatafs )
  ...
  const handlePincodeChange = async (event) => {
    ...
    if (pincode.length === 6) {
      ...
      fetch("https://www.relainstitute.in/relalive/api/HIS/GetDatabyPinCode", {
        method: "POST",
                ...
                <Row>
                  ...
                  <Form.Group as={Col} className="input-label">
                    <Form.Label className="label" style={{ width: "100px" }}>
                      ID Proof <span className="mandatory">*</span>
